...
function displayConfig() {
    if (DEBUG)console.log('config.displayConfig()');
        ...
        myEmitter.emit('log', 'config.displayConfig()', 'INFO', 'display config.json');
        ...
        function resetConfig(){
          ...
          fs.writeFile(__dirname + '/json/config.json', configdata, (err) => {
            ...
            myEmitter.emit('log', 'config.resetConfig()', 'INFO', 'Configuration returned to Normal');
    });
}
}function setConfig() {
    ...
    fetch.readFile(__dirname + "/json/config.json", (error, data)=> {
        ...
        if(!match){
            ...
            myEmitter.emit('log', config.setConfig(), 'Warning', 'invalid key: ${myArgs[2]}, try another.');
        }
